...
export default function MyBookings() {
  ...
  useEffect(() => { getBookings().then((response) => setBookings(response.bookings || [])).catch((requestError) => { if (requestError.response?.status === 401) { navigate('/', { replace: true }); return }; setError('We could not load your bookings right now.') }).finally(() => setLoading(false)) }, [navigate])
  return <section className="booking-list-page"><header className="page-heading"><h1>My bookings</h1></header><div className="booking-list-content">{loading && <p className="loading-message">Loading bookings...</p>}{error && <p className="form-error" role="alert">{error}</p>}{!loading && !error && !bookings.length && <div className="empty-state"><i className="ri-calendar-line" aria-hidden="true" /><h2>No bookings yet</h2><p>Choose a service when you are ready to request help.</p><Link className="primary-button" to="/home">Find a service</Link></div>}{bookings.map((booking) => <Link className="booking-list-card" key={booking._id} to={`/bookings/${booking._id}`}><div><b>{booking.serviceName}</b><small>{booking._id}</small></div><span className={`status-badge status-${booking.status}`}>{statusLabel(booking.status)}</span><small>Booked {formatDate(booking.createdAt)} · Expected by {formatDate(booking.serviceDeadlineAt)}</small><i className="ri-arrow-right-s-line" aria-hidden="true" /></Link>)}</div></section>
}
